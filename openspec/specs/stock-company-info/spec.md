...
Define authenticated stock company information APIs backed by `vnstock`
`Company(source='VCI')`, with tab-wise responses, canonical VCI field mapping,
symbol validation against the shared stock catalog, and per-symbol caching with
stale fallback.
