...
@pytest.fixture
def runtime_module():
    import app.agents.implementations.lead_agent.runtime as runtime

    runtime.get_lead_agent_runtime_catalog.cache_clear()
    yield runtime
    runtime.get_lead_agent_runtime_catalog.cache_clear()
...
def test_resolve_runtime_requires_reasoning_for_models_that_support_it(
    ...
    monkeypatch.setattr(
        ...
        lambda: (
            runtime_module.LeadAgentProviderCatalogEntry(
                provider="openai",
                display_name="OpenAI",
                is_default=True,
                models=(
                    runtime_module.LeadAgentModelCatalogEntry(
                        model="gpt-5.2",
    ...
    with pytest.raises(AppException) as exc_info:
        runtime_module.resolve_lead_agent_runtime_config(
            provider="openai",
            model="gpt-5.2",
            reasoning=None,
        )
...
def test_resolve_runtime_requires_provider_and_model(
    ...
    assert missing_provider_exc.value.message == "Lead-agent provider is required"
    assert missing_model_exc.value.message == "Lead-agent model is required"
...
def test_resolve_runtime_allows_null_reasoning_for_models_without_reasoning_support(
    ...
    runtime_config = runtime_module.resolve_lead_agent_runtime_config(
        provider="openai",
        model="gpt-4.1",
        reasoning=None,
    )

    assert runtime_config == runtime_module.LeadAgentRuntimeConfig(
        provider="openai",
        model="gpt-4.1",
        reasoning=None,
    )
...
def test_runtime_catalog_exposes_gpt_5_5(runtime_module) -> None:
    catalog = runtime_module.get_lead_agent_runtime_catalog()
    ...
    fast_model = next(
        model for model in openai_provider.models if model.model == "gpt-5.5-fast"
    )
