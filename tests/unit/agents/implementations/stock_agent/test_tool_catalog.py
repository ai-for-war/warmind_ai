...
import app.agents.implementations.stock_agent.tool_catalog as tool_catalog_module
...
def test_tool_catalog_exposes_research_tools_when_available(monkeypatch) -> None:
    ...
    catalog = tool_catalog_module.get_stock_agent_selectable_tool_catalog()
    runtime_tools = tool_catalog_module.get_stock_agent_tools()
...
def test_tool_catalog_is_empty_when_research_tools_are_unavailable(
    ...
    assert tool_catalog_module.get_stock_agent_selectable_tool_catalog() == []
    assert tool_catalog_module.get_stock_agent_selectable_tool_names() == set()
    assert [tool.name for tool in tool_catalog_module.get_stock_agent_tools()] == [
        "load_skill",
        "delegate_tasks",
    ]
...
def test_tool_catalog_exposes_only_available_research_tools(monkeypatch) -> None:
    ...
    assert tool_catalog_module.get_stock_agent_selectable_tool_names() == {"search"}
    assert [tool.name for tool in runtime_tools] == [
        "load_skill",
        "delegate_tasks",
        "search",
