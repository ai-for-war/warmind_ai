...
from app.services.ai.stock_agent_skill_access_resolver import StockAgentSkillAccessResolver
...
def _skill(skill_id: str) -> StockAgentSkill:
    now = datetime.now(timezone.utc)
    return StockAgentSkill(
        _id=f"{skill_id}-id",
        skill_id=skill_id,
        name=skill_id.title(),
...
async def test_resolver_prefers_organization_record_and_filters_unknown_skills() -> None:
    ...
    resolver = StockAgentSkillAccessResolver(
