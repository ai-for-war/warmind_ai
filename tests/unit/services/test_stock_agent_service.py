...
import app.services.ai.stock_agent_service as stock_agent_service_module
...
def _conversation(
    *,
    ...
    title: str = "Stock Agent Conversation",
...
def test_service_builds_distinct_agent_variants_for_direct_and_subagent_turns(
    ...
    def _fake_create_stock_agent(runtime_config=None, *, subagent_enabled=False):
        del runtime_config
        created_agents.append(subagent_enabled)
        return f"agent-{subagent_enabled}"

    monkeypatch.setattr(
        stock_agent_service_module,
        "create_stock_agent",
        _fake_create_stock_agent,
    )
...
async def test_send_message_rejects_unknown_unauthorized_or_non_stock_agent_conversations(
    ...
    with pytest.raises(StockAgentConversationNotFoundError):
        await service.send_message(
            user_id="user-1",
            content="follow-up",
            conversation_id="conv-1",
...
async def test_process_agent_response_emits_socket_lifecycle_and_persists_assistant_message(
    ...
    service.runtime_config = stock_agent_service_module.StockAgentRuntimeConfig(
        provider="openai",
        model="gpt-5.2",
        reasoning="medium",
    )
...
def test_message_content_to_stream_token_preserves_whitespace_for_structured_content() -> None:
    token = StockAgentService._message_content_to_stream_token(
        {
            "content": [
                {"text": "EM"},
                {"text": " trong"},
...
def test_serialize_tool_arguments_falls_back_for_non_json_runtime_objects() -> None:
    ...
    serialized = StockAgentService._serialize_tool_arguments(tool_input)
...
def test_serialize_tool_arguments_filters_injected_runtime_argument() -> None:
    serialized = StockAgentService._serialize_tool_arguments(
        {
            "url": "https://filum.ai/",
            "fmt": "text_markdown",
            "runtime": "ToolRuntime(...)",
...
def test_extract_token_usage_falls_back_to_response_metadata_token_usage() -> None:
    usage = StockAgentService._extract_token_usage(
        {
            "response_metadata": {
                "token_usage": {
                    "prompt_tokens": 13,
