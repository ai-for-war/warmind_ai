"""Todo middleware for the stock-agent runtime."""
...
from app.agents.implementations.stock_agent.middleware.constants import (
    STOCK_AGENT_TODO_REVISION_MARKER_PREFIX,
    STOCK_AGENT_TODO_STATE_MAX_ITEMS,
    TODO_STATUSES,
)
...
class StockAgentTodoMiddleware(TodoListMiddleware):
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__(
            system_prompt=system_prompt or get_stock_agent_todo_system_prompt(),
            tool_description=tool_description or get_stock_agent_todo_tool_description(),
        )
    ...
    *,
    ...
    return ToolMessage(
        content=(
            ...
            f"{STOCK_AGENT_TODO_REVISION_MARKER_PREFIX}{revision}]"
        ),
        tool_call_id=tool_call_id,
        additional_kwargs={
...
def _render_todo_state_prompt(todos: Any) -> str | None:
    ...
    rendered_todos = normalized_todos[:STOCK_AGENT_TODO_STATE_MAX_ITEMS]
...
def _extract_todo_revision(message: ToolMessage) -> int | None:
    ...
    content = str(message.content)
    marker_index = content.find(STOCK_AGENT_TODO_REVISION_MARKER_PREFIX)
    if marker_index < 0:
        return None

    start_index = marker_index + len(STOCK_AGENT_TODO_REVISION_MARKER_PREFIX)
