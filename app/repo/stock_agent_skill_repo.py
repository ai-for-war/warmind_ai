"""Repository for persisted user-created stock-agent skills."""
...
class StockAgentSkillListResult(BaseModel):
    """Paginated stock-agent skill query result."""

    items: list[StockAgentSkill]
    total: int


class StockAgentSkillRepository:
    """Persist and resolve user-created stock-agent skills."""

    def __init__(self, db: AsyncIOMotorDatabase):
        """Initialize the repository with the shared Mongo database."""
        self.collection = db.stock_agent_skills
    ...
    async def list_accessible(
        ...
        *,
        ...
        skills: list[StockAgentSkill] = []
        async for doc in cursor:
            doc["_id"] = str(doc["_id"])
            skills.append(StockAgentSkill(**doc))
        return skills
    ...
    async def list_by_creator(
        ...
        *,
        ...
        async for doc in cursor:
            doc["_id"] = str(doc["_id"])
            items.append(StockAgentSkill(**doc))

        return StockAgentSkillListResult(items=items, total=total)
    ...
    def _access_query(
        *,
        ...
        normalized_skill_ids = StockAgentSkillRepository._normalize_skill_ids(skill_ids)
