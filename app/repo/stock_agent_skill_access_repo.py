"""Repository helpers for organization-scoped stock-agent skill access persistence."""
...
class StockAgentSkillAccessRepository:
    """Persist per-user, per-organization stock-agent skill access records."""

    def __init__(self, db: AsyncIOMotorDatabase):
        """Initialize the repository with the shared Mongo database."""
        self.collection = db.stock_agent_skill_access
    ...
    async def get_by_scope(
        ...
        doc["_id"] = str(doc["_id"])
        return StockAgentSkillAccess(**doc)

    async def list_by_user(self, user_id: str) -> list[StockAgentSkillAccess]:
        ...
        records: list[StockAgentSkillAccess] = []
        async for doc in cursor:
            doc["_id"] = str(doc["_id"])
            records.append(StockAgentSkillAccess(**doc))
        return records
