"""Stock-agent service for checkpoint-backed conversation projections."""
...
from app.repo.stock_agent_conversation_repo import StockAgentConversationSearchResult
...
STOCK_AGENT_RECURSION_LIMIT = 200
...
class StockAgentService:
    """Manage stock-agent conversations backed by LangGraph checkpoints."""

    def __init__(
        self,
        conversation_service: StockAgentConversationService,
        skill_access_resolver: StockAgentSkillAccessResolver | None = None,
        runtime_config: StockAgentRuntimeConfig | None = None,
    ) -> None:
        """Initialize the service with stock-agent persistence helpers."""
    ...
    def agent(self) -> CompiledStateGraph:
        """Return the lazily built stock-agent runtime for this service instance."""
        if self._agent is None:
            self._agent = create_stock_agent(self.runtime_config)
        return self._agent

    def _get_agent(self, *, subagent_enabled: bool = False) -> CompiledStateGraph:
        """Return the compiled stock-agent runtime variant for the current turn."""
        if not subagent_enabled:
            return self.agent

        if self._subagent_agent is None:
            self._subagent_agent = create_stock_agent(
                self.runtime_config,
    ...
    def configure_runtime(
        ...
        *,
        ...
        self.runtime_config = resolve_stock_agent_runtime_config(
            provider=provider,
            model=model,
            reasoning=reasoning,
        )
    ...
    async def send_message(
        ...
        """Persist a stock-agent user turn and return message/conversation IDs."""
        normalized_content = self._normalize_content(content)
    ...
    async def process_agent_response(
        ...
        """Run the stock-agent runtime in the background and stream socket events."""
        try:
            ...
            _, thread_id = await self._require_stock_agent_conversation(
                conversation_id=conversation_id,
                user_id=user_id,
                organization_id=organization_id,
            ...
            logger.info(
                "Completed stock-agent turn for conversation %s (thread_id=%s, skill_id=%s, skill_version=%s, loaded_skills=%s, tool_calls=%d)",
                conversation_id,
                thread_id,
            ...
            logger.exception(
                "Error processing stock-agent response for conversation %s",
                conversation_id,
            )
            await gateway.emit_to_user(
                user_id=user_id,
    ...
    async def search_conversations(
        ...
        """Return only stock-agent conversations for the authenticated caller."""
    ...
    async def get_conversation_messages(
        ...
        """Return persisted messages for a stock-agent conversation."""
        conversation, _ = await self._require_stock_agent_conversation(
            conversation_id=conversation_id,
            user_id=user_id,
            organization_id=organization_id,
    ...
    async def get_conversation_plan(
        ...
        """Return the latest persisted todo snapshot for a stock-agent conversation."""
        _, thread_id = await self._require_stock_agent_conversation(
            conversation_id=conversation_id,
            user_id=user_id,
            organization_id=organization_id,
    ...
    async def _get_or_create_conversation_projection(
    ...
    ) -> tuple[StockAgentConversation, str]:
        """Create or load the stock-agent conversation projection for a send."""
        ...
        return await self._require_stock_agent_conversation(
            conversation_id=conversation_id,
            user_id=user_id,
            organization_id=organization_id,
            validate_thread_state=True,
    ...
    async def _require_stock_agent_conversation(
        self,
        conversation_id: str,
        user_id: str,
        organization_id: Optional[str],
        ...
        except (InvalidStockAgentThreadError, StockAgentThreadNotFoundError) as exc:
            raise StockAgentConversationNotFoundError() from exc
    ...
    async def _stream_thread_execution(
        ...
        """Stream the stock-agent runtime and emit chat-compatible socket events."""
    ...
    async def _require_user_message(
        ...
        if message is None:
            raise StockAgentRunError("Stock-agent user message not found")
        if message.conversation_id != conversation_id:
            raise StockAgentRunError(
                "Stock-agent user message does not match conversation"
            )
        if message.role != MessageRole.USER:
            raise StockAgentRunError("Stock-agent user message is not a user turn")
        if message.thread_id != thread_id:
            raise StockAgentRunError("Stock-agent user message does not match thread")
        return message
    ...
    async def _get_thread_state(self, thread_id: str) -> dict[str, Any]:
        ...
        if not values:
            raise StockAgentThreadNotFoundError()
    ...
    def _thread_config(thread_id: str) -> dict[str, Any]:
        ...
        return {
            ...
            "recursion_limit": STOCK_AGENT_RECURSION_LIMIT,
        }
    ...
    def _normalize_thread_id(thread_id: str) -> str:
        ...
        try:
            ...
            raise InvalidStockAgentThreadError() from exc
    ...
    def _serialize_tool_arguments(tool_input: Any) -> dict[str, Any]:
        ...
        encoded_input = StockAgentService._make_json_safe(tool_input)
    ...
    def _build_assistant_metadata(
        *,
        ...
        runtime_config: StockAgentRuntimeConfig | None,
        ...
        metadata = MessageMetadata(
            ...
            or None,
            skill_id=StockAgentService._normalize_optional_string(
                final_state.get("active_skill_id")
            ),
            skill_version=StockAgentService._normalize_optional_string(
                final_state.get("active_skill_version")
            ),
            loaded_skills=StockAgentService._normalize_string_list(
                final_state.get("loaded_skills", [])
            )
            or None,
            subagent_enabled=StockAgentService._normalize_optional_bool(
                final_state.get("subagent_enabled")
            ),
            orchestration_mode=StockAgentService._normalize_optional_string(
                final_state.get("orchestration_mode")
            ),
            delegation_depth=StockAgentService._normalize_optional_int(
                final_state.get("delegation_depth")
            ),
            delegation_parent_run_id=StockAgentService._normalize_optional_string(
                final_state.get("delegation_parent_run_id")
            ),
            delegated_execution_metadata=StockAgentService._normalize_optional_mapping(
    ...
    async def _resolve_skill_access_for_turn(
        ...
        *,
        ...
        if self.skill_access_resolver is None or organization_id is None:
            return ResolvedStockAgentSkillAccess(enabled_skill_ids=[])
